...
class TaskListViewModelTest {
    ...
    fun `GIVEN user intent create task WHEN createTask() THEN state SubmittedSuccess`() = runTest {
        ...
        assertEquals(TaskListState.SubmittedSuccess, taskListViewModel.state.value)
        coVerifyAll {
    ...
    fun `GIVEN user intent create task WHEN createTask() throws exception THEN state Error `() =
        runTest {
            ...
            assertEquals(TaskListState.Error(exception), taskListViewModel.state.value)
    ...
    fun `GIVEN user intent update task WHEN updateTask() THEN state UpdateSuccess  `() =
        runTest {
            ...
            assertEquals(TaskListState.UpdateSuccess, taskListViewModel.state.value)
    ...
    fun `GIVEN user intent delete task WHEN deleteTask() THEN state DeleteSuccess `() =
        runTest {
            ...
            assertEquals(TaskListState.DeleteSuccess, taskListViewModel.state.value)
            coVerifyAll { deleteTaskUseCase(0, ofType(Task::class)) }
        }
    ...
    fun `GIVEN user intent get task WHEN getTasks() THEN state Success `() =
        runTest {
            ...
            assertEquals(TaskListState.Success(tasks), taskListViewModel.state.value)
            coVerifyOrder {
                getAllPets()
                getAllTasksUseCase()
            }
